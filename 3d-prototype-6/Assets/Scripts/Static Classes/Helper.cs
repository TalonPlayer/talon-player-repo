...
{
    /// <summary>
    /// Returns a normalized Vector3 in a random ranged upwards direction
    /// </summary>
    /// <param name="minAngle">Angle in Degrees</param>
    /// <param name="maxAngle">Angle in Degrees</param>
    /// <returns></returns>
    public static Vector3 RandomLaunchDirection(float minAngle, float maxAngle)
    {
    ...
    public static Transform GetTransformChild(Transform parent, string name)
    {
        ...
        return allDescendants.FirstOrDefault(t => t.name == name);
    }

    public static Vector3 SpreadRandomizer(Vector3 dir, float spread)
    {
        dir += new Vector3(
